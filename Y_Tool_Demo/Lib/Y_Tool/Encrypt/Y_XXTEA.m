/**********************************************************\
 ...
 | Y_XXTEA.m                                                  |
 |                                                          |
 | Y_XXTEA encryption algorithm library for Objective-C.      |
 |                                                          |
...
static uint32_t * Y_XXTEA_to_uint_array(const uint8_t * data, size_t len, int inc_len, size_t * out_len) {
    uint32_t *out;
    size_t n;
...
static uint8_t * Y_XXTEA_to_ubyte_array(const uint32_t * data, size_t len, int inc_len, size_t * out_len) {
    uint8_t *out;
    size_t m, n;
...
static uint32_t * Y_XXTEA_uint_encrypt(uint32_t * data, size_t len, uint32_t * key) {
...
static uint32_t * Y_XXTEA_uint_decrypt(uint32_t * data, size_t len, uint32_t * key) {
    uint32_t n = (uint32_t)len - 1;
...
static uint8_t * Y_XXTEA_ubyte_encrypt(const uint8_t * data, size_t len, const uint8_t * key, size_t * out_len) {
    uint8_t *out;
    ...
    data_array = Y_XXTEA_to_uint_array(data, len, 1, &data_len);
    if (!data_array) return NULL;
    
    key_array  = Y_XXTEA_to_uint_array(key, 16, 0, &key_len);
    if (!key_array) {
        free(data_array);
        return NULL;
    }
    
    out = Y_XXTEA_to_ubyte_array(Y_XXTEA_uint_encrypt(data_array, data_len, key_array), data_len, 0, out_len);
...
static uint8_t * Y_XXTEA_ubyte_decrypt(const uint8_t * data, size_t len, const uint8_t * key, size_t * out_len) {
    uint8_t *out;
    uint32_t *data_array, *key_array;
    size_t data_len, key_len;
    ...
    data_array = Y_XXTEA_to_uint_array(data, len, 0, &data_len);
    if (!data_array) return NULL;
    ...
    out = Y_XXTEA_to_ubyte_array(Y_XXTEA_uint_decrypt(data_array, data_len, key_array), data_len, 1, out_len);
...
@implementation Y_XXTEA

+ (NSData *) encrypt:(NSData *)data key:(NSData *)key {
    size_t out_len;
    FIXED_KEY
    void * bytes = Y_XXTEA_ubyte_encrypt(data.bytes, data.length, fixed_key, &out_len);
...
+ (NSData *) decrypt:(NSData *)data key:(NSData *)key {
    size_t out_len;
    FIXED_KEY
    void * bytes = Y_XXTEA_ubyte_decrypt(data.bytes, data.length, fixed_key, &out_len);
...
@implementation NSData (Y_XXTEA)

- (NSData *) Y_XXTEAEncrypt:(NSData *)key {
    return [Y_XXTEA encrypt:self key:key];
}

- (NSData *) Y_XXTEADecrypt:(NSData *)key {
    return [Y_XXTEA decrypt:self key:key];
}
